...
export default function AdminPage() {
  ...
  const [form, setForm] = useState({
    nm_jogo: '', img_jogo: '', genero: '', plataforma: '',
    classificacao: '', jogadores: '', descricao: '', dt_jogo: '',
  });
  ...
  async function handleSubmit() {
    if (!form.nm_jogo || !form.img_jogo || !form.dt_jogo) {
      return toast('Nome, imagem e data são obrigatórios.', 'error');
    }
    setLoading(true);
    try {
      if (editId) {
  ...
  function handleEdit(j: Jogo) {
    ...
    setForm({
      ...
      jogadores:     (j as { jogadores?: string }).jogadores || '',
      descricao:     j.descricao      || '',
      dt_jogo:       j.dt_jogo ? j.dt_jogo.split('T')[0] : '',
    });
    setTab('jogos');
  }
    ...
    <div className="space-y-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500">Gerenciamento do sistema</p>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {(['dashboard', 'jogos'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`rounded-xl px-4 py-2.5 text-sm font-bold transition capitalize ${
      ...
      {tab === 'jogos' && (
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="card rounded-2xl p-6 space-y-4">
            ...
            <Input label="Nome *" value={form.nm_jogo}
              onChange={e => setForm(f => ({ ...f, nm_jogo: e.target.value }))}
              placeholder="Ex: Elden Ring" />
            <Input label="URL da imagem *" value={form.img_jogo}
              onChange={e => setForm(f => ({ ...f, img_jogo: e.target.value }))}
              placeholder="https://..." />

            {/* FIX: grid 2 colunas apenas para Gênero + Classificação */}
            <div className="grid grid-cols-2 gap-3">
              <Input label="Gênero" value={form.genero}
                onChange={e => setForm(f => ({ ...f, genero: e.target.value }))}
                placeholder="RPG de Ação" />
              <Input label="Classificação" value={form.classificacao}
                onChange={e => setForm(f => ({ ...f, classificacao: e.target.value }))}
                placeholder="18+" />
            </div>

            {/* FIX: Jogadores em linha própria (antes estava no grid de 2 col, quebrando o layout) */}
            <Input label="Jogadores" value={form.jogadores}
              onChange={e => setForm(f => ({ ...f, jogadores: e.target.value }))}
              placeholder="Solo / Multiplayer" />

            <Input label="Plataformas" value={form.plataforma}
              onChange={e => setForm(f => ({ ...f, plataforma: e.target.value }))}
              placeholder="PC / PS5 / Xbox" />
            <Input label="Data de lançamento *" type="date" value={form.dt_jogo}
              onChange={e => setForm(f => ({ ...f, dt_jogo: e.target.value }))} />
                  ...
                  <div key={j.id_jogo} className="card flex items-center gap-3 rounded-xl p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-zinc-500">
                        {j.genero} · {new Date(j.dt_jogo).getFullYear()}
                      </p>
                    </div>
                    <div className="flex gap-2 flex-shrink-0">
                      <button onClick={() => handleEdit(j)}
                        className="rounded-lg bg-zinc-800 px-2.5 py-1.5 text-xs hover:bg-zinc-700 transition">
                        Editar
                      </button>
                      <button onClick={() => handleDelete(j.id_jogo)}
                        className="rounded-lg bg-zinc-800 px-2.5 py-1.5 text-xs text-red-400 hover:bg-red-900/20 transition">
                        Excluir
                      </button>
                    </div>
                  </div>
