...
function UserCard({ user }: { user: UsuarioCard }) {
  return (
    <Link to={`/usuarios/${user.id_usuario}`} className="card card-hover flex items-center gap-3 rounded-2xl p-4">
      <Avatar src={user.img_usuario} name={user.nm_usuario} size="md" />
      <div className="min-w-0">
        <p className="font-bold truncate hover:text-checkpoint-green transition-colors">@{user.nm_usuario}</p>
        <p className="text-xs text-zinc-500">{user._count?.avaliacoes||0} avaliações · {user._count?.seguidores||0} seguidores</p>
      </div>
    </Link>
  );
}
  ...
  return (
    <div ref={wrapRef} className="relative">
      <div className="flex items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-950 px-3 focus-within:border-checkpoint-green transition-colors">
        ...
        />
        {q && <button onClick={() => { setQ(''); setOpen(false); }} className="text-zinc-500 hover:text-white"><X size={14} /></button>}
      </div>
...
export default function Feed() {
  ...
  const [tab, setTab] = useState<'following'|'discover'>(isAuthenticated ? 'following' : 'discover');

  const following = useQuery<Avaliacao[]>({ queryKey:['feed','following'], queryFn:()=>api.get('/feed/following').then(r=>r.data), enabled:isAuthenticated });
  const discover  = useQuery<DiscoverData>({ queryKey:['feed','discover'], queryFn:()=>api.get('/feed/discover').then(r=>r.data), staleTime:60_000 });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex gap-2">
        {isAuthenticated && (
          <button onClick={() => setTab('following')} className={`rounded-xl px-5 py-2.5 text-sm font-bold transition ${tab==='following'?'bg-checkpoint-green text-black':'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'}`}>Seguindo</button>
        )}
        <button onClick={() => setTab('discover')} className={`rounded-xl px-5 py-2.5 text-sm font-bold transition ${tab==='discover'?'bg-checkpoint-green text-black':'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'}`}>Descobrir</button>
      </div>
      ...
      {tab==='following' && (
        <>
          {following.isLoading ? (
            ...
            <EmptyState title="Nenhuma atividade ainda" description="Siga outros jogadores para ver suas avaliações aqui."
              action={<button onClick={() => setTab('discover')} className="rounded-xl bg-checkpoint-green px-5 py-2.5 text-sm font-bold text-black hover:brightness-110 transition">Descobrir jogadores</button>} />
          ) : (
            <div className="space-y-4">{following.data.map(r => <ReviewCard key={r.id_avaliacao} review={r} />)}</div>
          )}
        </>
      )}
          ...
          {(discover.data?.users?.length??0) > 0 && (
            <Section title="Jogadores da comunidade">
              <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {discover.data!.users.slice(0,8).map(u => <UserCard key={u.id_usuario} user={u} />)}
              </div>
            </Section>
          )}
          ...
          {(discover.data?.games?.length??0) > 0 && (
            <Section title="Novidades no catálogo">
              <div className="grid gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                {discover.data!.games.slice(0,10).map(g => <GameCard key={g.id_jogo} game={g} />)}
              </div>
            </Section>
          )}
        </div>
