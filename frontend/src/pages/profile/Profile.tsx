 ...
 * Perfil — v1.9
 * Biblioteca removida:
 *   - Stats: Avaliações | No diário | Listas | Nota média
 *   - vitrine vem de perfil.vitrine (era status_jogos)
 *   - Sem referências a status/favorito
 */

import { useState } from 'react';
import { useParams, Link } from 'react-router-dom';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { UserPlus, UserMinus, Star, Gamepad2, List, Pencil } from 'lucide-react';
import { api } from '../../services/api';
import { useAuth } from '../../context/AuthContext';
import { useToast } from '../../context/ToastContext';
import { DiarioEntry, Usuario } from '../../types';
...
export default function Profile() {
  ...
  // v1.9: vitrine vem diretamente do backend (apenas top_position ≠ null)
  const vitrine  = perfil.vitrine || [];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="card rounded-3xl p-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                ...
                {perfil.tipo_usuario === 'ADMIN' && (
                  <span className="mt-1 inline-block rounded-full bg-yellow-400/10 px-2 py-0.5 text-xs font-bold text-yellow-400">
                    Admin
                  </span>
                )}
              </div>
                ...
                {!isMe && (
                  <button
                    ...
                    className={`flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition ${
                      isFollowing
                        ? 'bg-zinc-800 text-zinc-300 hover:bg-red-900/20 hover:text-red-400'
            ...
            {perfil.bio_usuario && (
              <p className="mt-2 text-sm text-zinc-400">{perfil.bio_usuario}</p>
            )}
            <div className="mt-3 flex flex-wrap gap-5 text-sm">
              ...
              {avgNota && (
                <span>
                  <b className="font-black text-checkpoint-green">★ {avgNota}</b>{' '}
                  <span className="text-zinc-400">média</span>
                </span>
              )}
            </div>
          </div>
      ...
      {/* ── Stats — v1.9: baseados em atividade real ────────── */}
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { icon: <Star     size={16}/>, label: 'Avaliações', val: totalAvs                          },
          { icon: <Gamepad2 size={16}/>, label: 'No diário',  val: perfil._count?.diario     || 0    },
          { icon: <List     size={16}/>, label: 'Listas',     val: perfil._count?.listas     || 0    },
          { icon: <Star     size={16} fill="currentColor"/>, label: 'Nota média',
            val: avgNota ? `★ ${avgNota}` : '—' },
        ].map(({ icon, label, val }) => (
          ...
          {(perfil.avaliacoes?.length || 0) > 0 && (
            <div>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {perfil.avaliacoes!.slice(0, 4).map(a => (
                  <ReviewCard key={a.id_avaliacao} review={a}/>
                ))}
              </div>
      ...
      {tab === 'listas' && (
        <div>
          {!(perfil.listas?.length) ? (
            <p className="text-zinc-400">Nenhuma lista.</p>
          ) : (
                ...
                <Link key={l.id_lista} to={`/listas/${l.id_lista}`}
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-xs text-zinc-500">
                      {l.jogos?.length || 0} jogos · {l.likes_count || 0} ♥
                    </p>
                  </div>
                </Link>
              ))}
...
// ── Diário público ─────────────────────────────────────────
// v1.9: inclui entradas auto-criadas por avaliações
function DiarioPublico({ userId, isMe }: { userId: number; isMe: boolean }) {
  const { data: entries = [] } = useQuery<DiarioEntry[]>({
  ...
  if (!entries.length) return (
    <div className="card rounded-2xl p-8 text-center">
      <p className="text-zinc-400">Nenhuma entrada no diário ainda.</p>
      {isMe && (
        <Link to="/diario"
          className="mt-3 inline-block text-sm text-checkpoint-green hover:underline">
          Ir para o diário →
        </Link>
      )}
    </div>
  ...
  return (
    <div className="space-y-3">
      {entries.slice(0, 8).map((e: DiarioEntry) => (
        <div key={e.id_diario} className="card flex items-center gap-4 rounded-2xl p-3">
          <img
            ...
            className="h-14 w-10 rounded-lg object-cover object-top flex-shrink-0"
          />
          <div className="flex-1 min-w-0">
            <Link
              to={`/jogos/${e.jogo.id_jogo}`}
              className="font-bold text-sm hover:text-checkpoint-green transition-colors line-clamp-1"
            >
              {e.jogo.nm_jogo}
            </Link>
            <p className="text-xs text-zinc-500">
              {new Date(e.data_jogada).toLocaleDateString('pt-BR')}
            </p>
          </div>
          {e.nota && (
            <div className="flex-shrink-0">
              <Stars value={e.nota} size={12}/>
            </div>
          )}
        </div>
      ))}
      {entries.length > 8 && (
        <div className="text-center">
          {isMe ? (
            <Link to="/diario" className="text-sm text-zinc-500 hover:text-checkpoint-green transition-colors">
              Ver as {entries.length - 8} entradas restantes no diário completo →
            </Link>
          ) : (
            <p className="text-sm text-zinc-500">
              + {entries.length - 8} entradas no diário
            </p>
          )}
        </div>
      )}
